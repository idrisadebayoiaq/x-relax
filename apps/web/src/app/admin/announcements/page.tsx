  ...
  return (
    <div className="space-y-6 max-w-xl">
      ...
      <form onSubmit={onSubmit} className="card p-4 space-y-4">
        ...
        <input className="input" required value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Title" />
        <textarea className="input min-h-[100px]" required value={body} onChange={(e) => setBody(e.target.value)} placeholder="Message" />
        <button type="submit" className="btn btn-primary" disabled={busy}>{busy ? 'Sending…' : 'Send announcement'}</button>
