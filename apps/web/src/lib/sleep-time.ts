...
export function loadSleepTimeSchedule(userId: string): SleepTimeSchedule {
  if (typeof window === 'undefined') return { ...DEFAULT_SLEEP_TIME_SCHEDULE };
...
export function saveSleepTimeSchedule(userId: string, schedule: SleepTimeSchedule) {
  if (typeof window === 'undefined') return;
  localStorage.setItem(storageKey(userId), JSON.stringify(schedule));
}
