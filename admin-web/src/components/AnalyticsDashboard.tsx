...
import type { AnalyticsSummary, QueueStat } from '@/lib/analytics';
        ...
        {recent.map((day) => {
          ...
          return (
            <div key={day.day} className="flex-1 flex items-end justify-center gap-0.5 h-full" title={`${day.day}: ${a} ${aLabel}, ${b} ${bLabel}`}>
              <div className="w-[45%] min-h-px bg-foreground rounded-t-sm" style={{ height: `${Math.max(2, (a / max) * 100)}%` }} />
              <div className="w-[45%] min-h-px bg-foreground/40 rounded-t-sm" style={{ height: `${Math.max(2, (b / max) * 100)}%` }} />
            </div>
          );
        })}
      </div>
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <h1 className="text-3xl font-bold tracking-tight mt-1">How people find X-Relax</h1>
      ...
      <div>
        ...
        <h2 className="text-xl font-semibold mt-1 mb-4">Queues</h2>
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {queues.map((card) => (
            <Link key={card.href} href={card.href} className="rounded-2xl border border-border bg-surface p-5 hover:opacity-90">
